...
$(function () {
  ...
  function assignTask(option) {
    ...
    var $person =
      ...
      '<div class="avatar bg-light-warning"><div class="avatar-content">'+$(option.element).data('txt')+'</div></div>&nbsp;&nbsp;' +
   ...
   $.ajax({
        ...
        success:function(response){
         if(response.status == 1) {
                ...
                // }
                window.location.href = "/staff/todo";
            } else {
...
function updateTaskStatus(elem){
    ...
    $.ajax({
        type:'POST',
        url:'staff/todo/updatetaskstatus',
        data:{taskStatusArr},
        success:function(response){
